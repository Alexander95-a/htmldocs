...
<!--            <th>Noun</th>-->
        ...
        <?php
        $info = mysqli_query($conn, "SELECT * FROM `c3` INNER JOIN `tablo1` using (lot) WHERE `c3`.`uid`=2 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' AND `wc` = '$w' ");
...
<!--            <td></td>-->
            <td id="no"><?=$info['R3']?></td><!-- победа-->
            <td id="no"><?=$info['L3']?></td><!-- поражение-->
<!--            <td></td>-->
<!--            <td></td>-->
            <td></td>
            ...
            ?>
            <?php
            $info = mysqli_query($conn, "SELECT * FROM `c3` INNER JOIN `tablo1` using (lot) WHERE `c3`.`uid`=1 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' AND `wc` = '$w'  ");
...
<!--            <th>Tecnical actions</th>-->
        ...
        <?php
        $info1 = mysqli_query($conn, "SELECT * FROM `c3` INNER JOIN `tablo1`  using (lot) WHERE `c3`.`uid`= 3 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' AND `wc` = '$w' ");
...
<!--            <td></td>-->
<!--            <td></td>-->
            <td id="no"> <a href="../scripts/updC.php?l=c&x=2&y=3&n=3&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?= $sr ?>&cc=<?= $cc ?>&t=3">Update</a> </td>
            </tr>
    ...
    <a href="../scripts/ItogС.php?w=<?=$w?>&g=<?=$g?>&q=<?= $q ?>&z=<?= $z ?>" id="no" class="add">Итог</a>
<!--    <a href="../scripts/lone1C.php?r=5&w=--><?//=$w?><!--&g=--><?//=$g?><!--&q=--><?//= $q ?><!--&z=--><?//= $z ?><!--&id=2&n=4&sr=--><?//= $sr ?><!--&cc=--><?//= $cc ?><!--" class="button">END Round</a>-->
    </div>
