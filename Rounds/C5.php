...
//
//    if ($cc == 5) {
//        header("Location:../Rounds/C5.$cc.php?w=$w&g=$g&q=$q&z=$z&l=$l&n=$n&cc=$cc&sr=$sr");
//    }
//
...
<!--            <th>Noun</th>-->
        ...
        <?php
        $info = mysqli_query($conn, "SELECT * FROM `c5` INNER JOIN `tablo1` using (lot) WHERE `c5`.`uid`=2 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' AND `wc` = '$w' ");
...
<!--            <td></td>-->
            <td id="no"><?=$info['R5']?></td><!-- победа-->
            <td id="no"><?=$info['L5']?></td><!-- поражение-->
<!--            <td></td>-->
<!--            <td></td>-->
            <td  id="no"></td>
            ...
            ?>
            <?php
            $info = mysqli_query($conn, "SELECT * FROM `c5` INNER JOIN `tablo1` using (lot) WHERE `c5`.`uid`=5 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' AND `wc` = '$w'  ");
...
<!--            <td></td>-->
            ...
            ?>
            <?php
            $info = mysqli_query($conn, "SELECT * FROM `c5` INNER JOIN `tablo1` using (lot) WHERE `c5`.`uid`=1 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' AND `wc` = '$w'  ");
...
<!--            <td></td>-->
            ...
            ?>
<!--        <tr>-->
<!--            <th colspan="10" >M.Referee:--><?//= $e ?><!-- M.Secretary:--><?//= $f ?><!--</th>-->
<!---->
<!--        </tr>-->
    </table>
    <table>
...
<!--            <th>Tecnical actions</th>-->
        ...
        <?php
        $info1 = mysqli_query($conn, "SELECT * FROM `c5` INNER JOIN `tablo1`  using (lot) WHERE `c5`.`uid`= 4 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' AND `wc` = '$w' ");

        foreach ($info1 as $info1){
            ?>
            <td id="no"><?=$info1['R5']?></td><!-- победа-->
            <td id="no"><?=$info1['L5']?></td><!-- поражение-->
            <td><?=$info1['lot']?></td>
            <td><?=$info1['Name']?></td>
...
<!--            <td></td>-->
<!--            <td></td>-->
            <td id="no"> <a href="../scripts/updC.php?l=c&x=2&y=4&n=5&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?= $sr ?>&cc=<?= $cc ?>&t=3">Update</a> </td>
            </tr>
        ...
        ?>
        <?php
        $info1 = mysqli_query($conn, "SELECT * FROM `c5` INNER JOIN `tablo1`  using (lot) WHERE `c5`.`uid` = 3 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' AND `wc` = '$w' ");
...
<!--            <td></td>-->
<!--            <td></td>-->
            <td id="no"> <a href="../scripts/updC.php?l=c&x=5&y=3&n=5&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?= $sr ?>&cc=<?= $cc ?>&t=3">Update</a> </td>
            </tr>
...
<!--        </tr>-->
        ...
        <br>
    <a href="../scripts/ItogС.php?w=<?=$w?>&g=<?=$g?>&q=<?= $q ?>&z=<?= $z ?>" class="add" id="no">Итог</a>
