            ...
            <div>
                <td  style="background: #ffffff; vertical-align: top; padding: 1px;">
                    ...
                    <table width="600" style="float: right;">
                            ...
                            <?php
                            $info = mysqli_query($conn, "SELECT * FROM `uc` INNER JOIN `tablo1` using (lot) WHERE `uc`.`uid` % 2 <> 0 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' and `qga` like '$g' AND `wc` = '$w' and `qwc` = '$w'  ORDER BY `uc`.`lot` ASC ");
                ...
                <td style="background: #ffffff; vertical-align: top ; padding: 1px;">          
                    <table width="700" style="float: left;">
                        ...
                        <?php
                        $info1 = mysqli_query($conn, "SELECT * FROM `uc`  INNER JOIN `tablo1`  using (lot) WHERE `uc`.`uid` % 2 = 0 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' and `qga` like '$g' AND `wc` = '$w' and `qwc` = '$w'  ORDER BY `uc`.`lot` ASC ");
                        ...
                        foreach ($info1 as $info1) {
                            ...
                            <?php
                                ...
                                <td <?php 
                                    $jr = $info1['lot'];
                                    $info2 = mysqli_query($conn, "SELECT * FROM `uc` INNER JOIN `tablo1` using (lot) WHERE `uc`.`lot`= '$jr'  AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' AND `wc` = '$w'");
                                    $info2 = mysqli_fetch_assoc($info2);
                                    $jq = $info2["RC"];
                                    $cn = $info2["LC"];
?>
                                    style="text-align: center; <?php
                                        if($cn == 2 || $jq == 2 ){?>
                                            ...
                                            ?>"> <a href="../scripts/updateUC.php?l=a&id=<?= $info1['lot'] ?>&n=1&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=3" class="link">Update</a>  </td>
                                <?php
                                }
                                ?>
                                <td><?=$info1["RC"]
                    ...
                    <a href="../Rounds/RU1.php?r=3&w=<?=$w?>&g=<?=$g?>&q=<?= $q ?>&z=<?= $z ?>&n=2&sr=<?=$sr?>" class="button" style="align: center">Завершить</a>
