...
if ($f3==2){
    if($cc == 4) {
        header("Location:../Rounds/RD4.php?w=$w&g=$g&q=$q&z=$z&l=$l&n=$n&sr=$sr");
    }
}
    if ($f3==1){
        if($cc == 2) {
        ...
        <tr>
            ...
            <th>Tecnical actions</th>
                ...
                ?>
                    ...
                    if ($sr>0) {
                        ?>
                        <td> <a href="../scripts/updD.php?l=<?=$l?>&gi=<?=$gi ?>&id=<?= $info1["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=3" class="link">Update</a> </td>
                        <?php
