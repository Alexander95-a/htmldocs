            ...
            <div>
                <td  style="background: #ffffff; vertical-align: top; padding: 1px;">
                    <table width="600" style="float: right;">
                            ...
                            <?php
                            $info = mysqli_query($conn, "SELECT * FROM `g1` INNER JOIN `tablo1` using (lot) WHERE `g1`.`uid` % 2 <> 0 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' and `qga` like '$g' AND `wc` = '$w' and `qwc` = '$w'  ORDER BY `g1`.`lot` ASC ");
                ...
                <td style="background: #ffffff; vertical-align: top ; padding: 1px;">   
                    ...
                    <table width="700" style="float: left;">
                        ...
                        <?php
                        $info1 = mysqli_query($conn, "SELECT * FROM `g1`  INNER JOIN `tablo1`  using (lot) WHERE `g1`.`uid` % 2 = 0 AND `lot` !=0 AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' and `qga` like '$g' AND `wc` = '$w' and `qwc` = '$w'  ORDER BY `g1`.`lot` ASC ");
                        ...
                        foreach ($info1 as $info1) {
                            ...
                            <?php
                                ...
                                <td <?php 
                                    $jr = $info1['lot'];
                                    $info2 = mysqli_query($conn, "SELECT * FROM `g1` INNER JOIN `tablo1` using (lot) WHERE `g1`.`lot`= '$jr'  AND `cid` = '$z' AND `qid` = '$z'  AND `gendage` LIKE '$g' AND `wc` = '$w'");
                                    $info2 = mysqli_fetch_assoc($info2);
                                    $jq = $info2["R1"];
                                    $cn = $info2["L1"];
                                    ?>
...
<br>
        ...
        <a href="../scripts/lone1.php?r=3&w=<?=$w?>&g=<?=$g?>&q=<?= $q ?>&z=<?= $z ?>&n=2&sr=<?=$sr?>" class="button" style="align: center">Завершить раунд</a>
