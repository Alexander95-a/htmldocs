...
mysqli_query($conn, "REPLACE INTO u1 ( uid,Name, Team, cid, gendage, wc, lot,uidb) SELECT id, Name, Team, cid, gendage, wc, lot,(`cid`+`lot`/'10000')  FROM r1 where `lot` !=0 AND `cid` = '$z'   AND `gendage` LIKE '$g' AND `wc` = '$w' ");
mysqli_query($conn, "UPDATE tablo1 JOIN u1 ON tablo1.lot=u1.lot and tablo1.qid=u1.cid  set tablo1.quid=u1.uid");
mysqli_query($conn, "UPDATE `u1` SET `uidb`= `cid`+`lot`/'10000' WHERE id =id");

mysqli_query($conn, "TRUNCATE person1");
mysqli_query($conn, "TRUNCATE r1");

header("Location:../Rounds/RU1.php?w=$w&g=$g&q=$q&z=$z&sr=$sr&l=g&n=1");