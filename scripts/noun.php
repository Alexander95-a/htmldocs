...
$info1 = mysqli_query($conn, "select * from `g1` INNER JOIN `tablo1` using (lot) where `uid` = (select max(uid) from `g1` where `uid` < '$info_id' AND `lot` != 0 AND `cid` = '$z' AND `gendage` LIKE '$g' AND `wc` = '$w')");
