...
$info1 = mysqli_query($conn, "select * from `$l$n` INNER JOIN `tablo1` using (lot) where `uid` = (select max(uid) from `$l$n` where `uid` < '$info_id' AND `lot` != 0 AND `cid` = '$z' AND `gendage` LIKE '$g' AND `wc` = '$w' AND `qid` = '$z' AND `qga` LIKE '$g' AND `qwc` = '$w')");
...
<head>
    ...
    <h3>Update point</h3>
        ...
        <form action="noun_algB.php" method="post">
