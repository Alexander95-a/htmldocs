...
if($para==0){
    mysqli_query($conn,"INSERT INTO `b$r` (`id`, `uid`, `Name`, `Team`, `lot2`, `lot`, `cid`, `gendage`, `wc`, `uidb`) SELECT NULL,'$uid1', '$N1', '$T1', '$id', '$id', '$z', '$g', '$w','$uidb1' FROM `tablo1` WHERE `R$di`= 2 AND `lot` = $id AND `qid` = '$z' AND `qga` LIKE '$g' AND `qwc` = '$w' ");
}
if($para1==0){
    mysqli_query($conn,"INSERT INTO `b$r` (`id`, `uid`, `Name`, `Team`, `lot2`, `lot`, `cid`, `gendage`, `wc`, `uidb`) SELECT NULL,'$uid2', '$N2', '$T2', '$id2', '$id2', '$z', '$g', '$w','$uidb2' FROM `tablo1` WHERE `R$di`= 2 AND `lot` = $id2 AND `qid` = '$z' AND `qga` LIKE '$g' AND `qwc` = '$w' ");
...
header("Location:../scripts/updb3.php?w=$w&g=$g&q=$q&z=$z&n=$di&l=$l&id=$uid1&gi=$gi&sr=$sr&t=1&ch1=$ch1&ch2=$ch2&ch3=$ch3&ch4=$ch4&o1=$o1");