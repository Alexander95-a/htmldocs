...
$info = mysqli_query($conn, "SELECT * FROM `d1` INNER JOIN `tablo1` using (lot) WHERE `tablo1`.`lot` = '$info_id' AND `cid` = '$z' AND `gendage` LIKE '$g' AND `wc` = '$w' and `qid` = '$z' AND `qga` LIKE '$g' AND `qwc` = '$w'");
$info1 = mysqli_query($conn, "select * from `d1` INNER JOIN `tablo1` using (lot) where d1.`cid` = '$z' `lot` = (select max(lot) from `tablo1` where `lot` < '$info_id' AND `lot` != 0 AND `qid` = '$z' AND `qga` LIKE '$g' AND `qwc` = '$w') AND `lot` != 0  AND `qid` = '$z'  AND `qga` LIKE '$g' AND `qwc` = '$w' ");
...
<!--            <td><a href="../scripts/med.php?l=a&n=--><?//=$n?><!--&id=--><?//= $info1['uid']?><!--&w=--><?//=$w?><!--&g=--><?//=$g?><!--&q=--><?//= $q ?><!--&z=--><?//= $z ?><!--" class="button">Med</a></td>-->
            <p>Предупреждения</p>
                <?php
                if ($ch1== "white"){?>
                    <a href="updateD.php?&t=1&l=<?=$l?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch1=yellow&ch2=<?= $ch2 ?>&ch3=<?= $ch3 ?>&ch4=<?= $ch4 ?>&o1=<?=$info1['lot']?>" class="button">1</a>
                    <?php
                }
                ?>
                <?php
                if ($ch1== "yellow"){?>
                    <a href="updateD.php?&t=1&l=<?=$l?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch1=yellow&ch2=<?= $ch2 ?>&ch3=<?= $ch3 ?>&ch4=<?= $ch4 ?>&o1=<?=$info1['lot']?>" class="<?= $ch1 ?>">1</a>
                    <?php
                }
                ?>
                <?php
                if ($ch2== "white"){?>
                    <a href="updateD.php?&t=1&l=<?=$l?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch2=red&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&ch3=<?= $ch3 ?>&o1=<?=$info1['lot']?>" class="button">2</a>
                    <?php
                }
                ?>
                <?php
                if ($ch2== "red"){?>
                    <a href="updateD.php?&t=1&l=<?=$l?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch2=red&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&ch3=<?= $ch3 ?>&o1=<?=$info1['lot']?>" class="<?= $ch2 ?>">2</a>
                    <?php
                }
                ?>


            <a href="updateD.php?&t=1&l=<?=$l?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch1=white&ch2=white&ch3=<?= $ch3 ?>&ch4=<?= $ch4 ?>&o1=<?=$info1['lot']?>" class="button">сброс</a>
                <p>Итог</p>
                <select name="para1">
...
<!--            <td><a href="../scripts/med.php?l=a&n=--><?//=$n?><!--&id=--><?//= $info['uid']?><!--&w=--><?//=$w?><!--&g=--><?//=$g?><!--&q=--><?//= $q ?><!--&z=--><?//= $z ?><!--" class="button">Med</a></td>-->
            <p>Предупреждения</p>
                <?php
                if ($ch3== "white"){?>
                    <a href="updateD.php?&t=1&l=<?=$l?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch3=yellow&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&o2=<?=$info['lot']?>" class="button">1</a>
                    <?php
                }
                ?>
                <?php
                if ($ch3== "yellow"){?>
                    <a href="updateD.php?&t=1&l=<?=$l?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch3=yellow&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&o2=<?=$info['lot']?>" class="<?= $ch3 ?>">1</a>
                    <?php
                }
                ?>
                <?php
                if ($ch4== "white"){?>
                    <a href="updateD.php?&t=1&l=<?=$l?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch4=red&ch1=<?= $ch1 ?>&ch2=<?= $ch2 ?>&ch3=<?= $ch3 ?>&o2=<?=$info['lot']?>" class="button">2</a>
                    <?php
                }
                ?>
                <?php
                if ($ch4== "red"){?>
                    <a href="updateD.php?&t=1&l=<?=$l?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch4=red&ch1=<?= $ch1 ?>&ch2=<?= $ch2 ?>&ch3=<?= $ch3 ?>&o2=<?=$info['lot']?>" class="<?= $ch4 ?>">2</a>
                    <?php
                }
                ?>
            ...
            <a href="updateD.php?&t=1&l=<?=$l?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch3=white&ch4=white&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&o2=<?=$info['lot']?>" class="button">сброс</a>
                <br>
                <p>Итог</p>
                <select name="para">
            ...
            <?php
            if ($t== 1){?>
                <a href="updateD.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=2&o1=<?=$info1['lot']?>" class="button">start</a>
                <a href="updateD.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=1&o1=<?=$info1['lot']?>" class="red">stop</a>
                <a href="updateD.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=3&o1=<?=$info1['lot']?>" class="button">reset</a>
            ...
            <?php
            if ($t== 2){?>
                <a href="updateD.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=2&o1=<?=$info1['lot']?>" class="add">start</a>
                <a href="updateD.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=1&o1=<?=$info1['lot']?>" class="button">stop</a>
                <a href="updateD.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&id=<?= $info["lot"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=3&o1=<?=$info1['lot']?>" class="button">reset</a>
            ...
            <a href="../Rounds/ED1.php?w=<?=$w?>&g=<?=$g?>&q=<?= $q ?>&z=<?= $z ?>&t1=<?= $info['lot'] ?>&t2=<?= $info1['lot'] ?>&sr=<?=$sr?>" class="button">back</a>
        </form>
<!--<p>--><?//= $id ?><!--</p>-->
<!--<p>--><?//= $id2 ?><!--</p>-->
<!--<p>--><?//= $c ?><!--</p>-->
