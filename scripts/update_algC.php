...
mysqli_query($conn,"UPDATE `tablo1` SET `R$di` = '$score', `O$di` = '$uid2' , `Scores` = `R1`, `Wins` = `Scores`, `L$di` = '$lose', `lose` = `L1`, `m$di` = `R$di` WHERE `tablo1`.`lot` = $id AND `qid` = '$z'AND `qga` LIKE '$g' AND `qwc` = '$w' ");
mysqli_query($conn,"UPDATE `tablo1` SET `R$di` = '$lose', `O$di` = '$uid1' , `Scores` = `R1`, `Wins` = `Scores`, `L$di` = '$score', `lose` = `L1`, `m$di` = `R$di` WHERE `tablo1`.`lot` = $id2 AND `qid` = '$z'AND `qga` LIKE '$g' AND `qwc` = '$w'");
...
header("Location:../scripts/updateC.php?w=$w&g=$g&q=$q&z=$z&id=$id&cc=$cc&sr=$sr&n=$di&&t=1&ch1=$ch1&ch2=$ch2&ch3=$ch3&ch4=$ch4&o1=$o1");