...
$info = mysqli_query($conn, "SELECT * FROM `person` INNER JOIN `tablo1` using (id) WHERE `tablo1`.`lot` = '$info_id' AND `cid` = '$z' AND `gendage` LIKE '$g' AND `wc` = '$w' ");
$info1 = mysqli_query($conn, "select * from `person` INNER JOIN `tablo1` using (id) where `lot` = (select min(lot) from `tablo1` where `lot` > '$info_id' AND `lot` != 0 AND `cid` = '$z' AND `gendage` LIKE '$g' AND `wc` = '$w')");
...
<head>
    ...
    <h3>Update point</h3>
        ...
        <form action="noun_alg.php" method="post">
