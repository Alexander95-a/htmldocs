...
$info = mysqli_query($conn, "SELECT * FROM `u$n` INNER JOIN `tablo1` using (lot) WHERE `u$n`.`uid` = '$info_id' AND `cid` = '$z' AND `gendage` LIKE '$g' AND `wc` = '$w' and `qid` = '$z' AND `qga` LIKE '$g' AND `qwc` = '$w'");
$info1 = mysqli_query($conn, "SELECT * FROM `u$n` INNER JOIN `tablo1` using (lot) where `u$n`.`uid` = (select max(uid) from `u$n` where `uid` < '$info_id' AND `lot` != 0 AND `cid` = '$z' AND `gendage` LIKE '$g' AND `wc` = '$w' and `qid` = '$z' AND `qga` LIKE '$g' AND `qwc` = '$w')");
...
action="updbu_alg.php" method="post">
    <h3>Панель управления поединком</h3>
    <h4>Б категория: раунд <?= $n ?></h4>
    ...
    <div style="float:left;">
        ...
        <?php
        if ($ch1== "white"){?>
            <a href="updu.php?l=<?=$l?>&t=1&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch1=yellow&ch2=<?= $ch2 ?>&ch3=<?= $ch3 ?>&ch4=<?= $ch4 ?>&o1=<?=$info1['lot']?>" class="button">1</a>
            <?php
        }
        ?>
        <?php
        if ($ch1== "yellow"){?>
            <a href="updu.php?l=<?=$l?>&t=1&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch1=yellow&ch2=<?= $ch2 ?>&ch3=<?= $ch3 ?>&ch4=<?= $ch4 ?>&o1=<?=$info1['lot']?>" class="<?= $ch1 ?>">1</a>
            <?php
        }
        ?>
        <?php
        if ($ch2== "white"){?>
            <a href="updu.php?l=<?=$l?>&t=1&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch2=red&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&ch3=<?= $ch3 ?>&o1=<?=$info1['lot']?>" class="button">2</a>
            <?php
        }
        ?>
        <?php
        if ($ch2== "red"){?>
            <a href="updu.php?l=<?=$l?>&t=1&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch2=red&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&ch3=<?= $ch3 ?>&o1=<?=$info1['lot']?>" class="<?= $ch2 ?>">2</a>
            <?php
        }
        ?>

        <a href="updu.php?&t=1&l=<?=$l?>&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch1=white&ch2=white&ch3=<?= $ch3 ?>&ch4=<?= $ch4 ?>&o1=<?=$info1['lot']?>" class="button">сброс</a>
    ...
    <div style="float:right;">
        ...
        <?php
        if ($ch3== "white"){?>
            <a href="updu.php?l=<?=$l?>&t=1&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch3=yellow&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&o2=<?=$info['lot']?>" class="button">1</a>
            <?php
        }
        ?>
        <?php
        if ($ch3== "yellow"){?>
            <a href="updu.php?l=<?=$l?>&t=1&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch3=yellow&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&o2=<?=$info['lot']?>" class="<?= $ch3 ?>">1</a>
            <?php
        }
        ?>
        <?php
        if ($ch4== "white"){?>
            <a href="updu.php?l=<?=$l?>&t=1&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch4=red&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch3=<?= $ch3 ?>&o2=<?=$info['lot']?>" class="button">2</a>
            <?php
        }
        ?>
        <?php
        if ($ch4== "red"){?>
            <a href="updu.php?l=<?=$l?>&t=1&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch4=red&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch3=<?= $ch3 ?>&o2=<?=$info['lot']?>" class="<?= $ch4 ?>">2</a>
            <?php
        }
        ?>

        <a href="updu.php?&t=1&l=<?=$l?>&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&ch3=white&ch4=white&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&o2=<?=$info['lot']?>" class="button">сброс</a>
        <br>
        <p>Итог</p>
        <select name="para">
    ...
    <?php
    if ($t== 1){?>
        <a href="updu.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=2&o1=<?=$info1['lot']?>" class="button">start</a>
        <a href="updu.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=1&o1=<?=$info1['lot']?>" class="red">stop</a>
        <a href="updu.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=3&o1=<?=$info1['lot']?>" class="button">reset</a>
    ...
    <?php
    if ($t== 2){?>
        <a href="updu.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=2&o1=<?=$info1['lot']?>" class="add">start</a>
        <a href="updu.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=1&o1=<?=$info1['lot']?>" class="button">stop</a>
        <a href="updu.php?l=<?=$l?>&ch3=<?= $ch3 ?>&ch2=<?= $ch2 ?>&ch1=<?= $ch1 ?>&ch4=<?= $ch4 ?>&gi=<?=$gi ?>&id=<?= $info["uid"] ?>&n=<?=$n?>&w=<?=$w?>&g=<?=$g?>&q=<?=$q?>&z=<?=$z?>&sr=<?=$sr?>&t=3&o1=<?=$info1['lot']?>" class="button">reset</a>
    ...
    <a href="../Rounds/EU.php?w=<?=$w?>&g=<?=$g?>&gi=<?=$gi?>&q=<?= $q ?>&z=<?= $z ?>&l=<?= $l ?>&n=<?= $n ?>&t1=<?= $info['lot'] ?>&t2=<?= $info1['lot'] ?>&sr=<?=$sr?> " class="button">back</a>
</form>
